...
TEST_CASE("HmmDisambiguationTest-testDisambiguation") {
    ...
    REQUIRE_THAT(0.9333, Catch::Matchers::WithinAbs((correctRoot + 0.0) / corpus.numberOfWords(), 0.002));
    REQUIRE_THAT(0.8651, Catch::Matchers::WithinAbs((correctParse + 0.0) / corpus.numberOfWords(), 0.002));
}